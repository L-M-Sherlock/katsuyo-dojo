...
2026-09-21 Windows 已完成 82 个协议 3 批次的全部合并和审核。综合运用仍为 1,301 张；当时全站 17,606 张，后续动作卡修订三张、撤回死ぬ的一张，当前候选共 17,605 张。综合运用历史发布范围见 [发布记录](integration-release.md) 和 [v3 账本](integration-stage-requirements.v3.json)；后续差异见 [动作审核账本](usage-card-actions-review.json)，不得覆盖原发布快照。

制作队列已经完成，开放项为 0；不要重启历史 `pool-loop.mjs`。被撤回的报告、失败稿和协调修复事件都留在任务历史中，最终证明只纳入有效独立审核结论。v1 冻结要求、v2 分批发布账本与证明均保持原始字节；v3 另存完整账本及证明。后续修订必须保留已发布内容的原始证据，不能重跑要求旧基线的历史导出脚本覆盖新运行时。

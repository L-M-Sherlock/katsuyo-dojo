...
| `restore-superseded --actor /root --batch lane/NN --stage ID --history <原退回记录> --reason ...` | 主代理 | 纠正误重开的历史失败阶段；只恢复有收据绑定的 rejected 状态，并要求后续 approved 快照已覆盖所有配对，随后仍须通过 merge 门禁 |

...
test("packs a second atom into the same round when enough focus slots remain", () => {
  ...
  const exercises = components.flatMap((kc) => Array.from({ length: 12 }, (_, index) => ({ id: `${kc.id}-${index}`, courseId: "course", courseIndex: 0, form: kc.id, verb: { surface: `${kc.id}-${index}` }, kcIds: [kc.id] })));

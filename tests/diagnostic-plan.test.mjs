...
test('all 170 current forms have declared plans and every representative accepted branch reaches the right answer',()=>{
  const courses=new Map(UNIFIED_COURSES.flatMap(c=>c.forms.map(form=>[form,c])));
  assert.equal(courses.size,170);assert.deepEqual(new Set(DIAGNOSTIC_FORMS),new Set(courses.keys()));

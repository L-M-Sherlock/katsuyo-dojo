...
import { CHINESE_YOKUBI_URL, CORE_COURSE_COUNT, COURSES as CURRICULUM_COURSES, componentsForScope, coursesForScope } from "./lib/curriculum.mjs";
...
const COURSES = CURRICULUM_COURSES as Course[];
...
const CORE_KCS = componentsForScope(ALL_KCS, "core") as KnowledgeComponent[];
...
const exerciseKey = (exercise: Exercise) => `${exercise.form ?? "classify"}:${exercise.verb.surface}`;
...
function kcsForScope(scope: CurriculumScope) { return scope === "core" ? CORE_KCS : ALL_KCS; }
function activateReadyKcs(profile: Profile, scope: CurriculumScope) {
  const advanced = advanceIntroductions(kcsForScope(scope), profile.introducedKcIds, profile.byKc) as { introducedKcIds: string[] };
  return { ...profile, introducedKcIds: advanced.introducedKcIds };
}
function makePlan(mode: PracticeMode, profile: Profile, length = SESSION_LENGTH, scope: CurriculumScope = "core") {
  const allowedIds = new Set(kcsForScope(scope).map((kc) => kc.id));
...
function loadProfile(scope: CurriculumScope): Profile {
  const fresh = emptyProfile();
  try {
    const saved = localStorage.getItem(STORAGE_KEY);
    if (saved) {
      const profile = activateReadyKcs(parseProfileImport(JSON.parse(saved), importOptions()) as Profile, scope);
      localStorage.setItem(STORAGE_KEY, JSON.stringify(profile));
      return profile;
    }
    const legacyV4 = localStorage.getItem(LEGACY_PROFILE_KEY_V4);
    if (legacyV4) {
      const profile = activateReadyKcs(parseProfileImport(JSON.parse(legacyV4), importOptions()) as Profile, scope);
      localStorage.setItem(STORAGE_KEY, JSON.stringify(profile));
      return profile;
    }
...
export default function Home() {
  ...
  const firstPlan = makePlan("adaptive", profile, SESSION_LENGTH, curriculumScope);
  ...
  const roundQuestions = useMemo(() => {
    ...
    const allowedIds = new Set(kcsForScope(curriculumScope).map((kc) => kc.id));
  ...
  }, [curriculumScope, focusKc, mode, planningProfile, roundPlan, seed, usedQuestionKeys]);
  ...
  const readingDetail = form ? explainConjugation(verb.reading, verb.class, form) : null;
  ...
  useEffect(() => { const migrated = !localStorage.getItem(STORAGE_KEY) && Boolean(localStorage.getItem(LEGACY_PROFILE_KEY_V4)); const scope = loadCurriculumScope(); const loaded = loadProfile(scope); profileRef.current = loaded; startedAt.current = clockNow(); const next = makePlan("adaptive", loaded, SESSION_LENGTH, scope); requestAnimationFrame(() => { setCurriculumScope(scope); setProfile(loaded); setPlanningProfile(loaded); setRoundPlan(next.plan); setFocusKc(next.focus); setMigrationNotice(migrated); }); }, []);
  ...
  const nextQuestion = useCallback(() => {
    ...
    if (focusKc && isComponentMastered(focusKc, current.byKc)) {
      const advanced = mode === "adaptive"
        ? advanceIntroductions(kcsForScope(curriculumScope), current.introducedKcIds, current.byKc) as { introducedKcIds: string[]; added: KnowledgeComponent[] }
      ...
      const next = makePlan(mode, nextProfile, remaining, curriculumScope);
    ...
    resetQuestion();
  }, [curriculumScope, finishRound, focusKc, mode, questionIndex, resetQuestion, roundOffset, roundQuestions, save, usedQuestionKeys]);
  ...
  function start(nextMode: PracticeMode, scope: CurriculumScope = curriculumScope) { const current = { ...profileRef.current, rotation: profileRef.current.rotation + 1 }; save(current); const next = makePlan(nextMode, current, SESSION_LENGTH, scope); setPlanningProfile(current); setMode(nextMode); setRoundPlan(next.plan); setFocusKc(next.focus); setQuestionIndex(0); setRoundOffset(0); setUsedQuestionKeys([]); setSessionCorrect(0); setFinished(false); setUnlocked(null); setSeed((value) => value + 1); resetQuestion(); }
  function changeCurriculumScope(nextScope: CurriculumScope) { if (nextScope === "full" && !CORE_GATING_KCS.every((kc) => isComponentMastered(kc, profileRef.current.byKc))) return; localStorage.setItem(CURRICULUM_SCOPE_KEY, nextScope); setCurriculumScope(nextScope); save(activateReadyKcs(profileRef.current, nextScope)); start("adaptive", nextScope); }
  function resetProgress() { if (!window.confirm("确定清除这台设备上的全部练习进度吗？")) return; localStorage.removeItem(LEGACY_PROFILE_KEY_V4); localStorage.removeItem(LEGACY_PROFILE_KEY_V3); localStorage.removeItem(LEGACY_PROFILE_KEY_V2); localStorage.removeItem("katsuyo-practice-stats-v1"); const fresh = emptyProfile(); save(fresh); const next = makePlan("adaptive", fresh, SESSION_LENGTH, curriculumScope); setPlanningProfile(fresh); setMode("adaptive"); setRoundPlan(next.plan); setFocusKc(next.focus); setQuestionIndex(0); setRoundOffset(0); setUsedQuestionKeys([]); setSessionCorrect(0); setFinished(false); setUnlocked(null); resetQuestion(); }
  ...
  async function importProgress(event: React.ChangeEvent<HTMLInputElement>) {
    ...
    if (!file) return;
    try {
      const imported = activateReadyKcs(parseProfileImport(JSON.parse(await file.text()), importOptions()) as Profile, curriculumScope);
      if (!window.confirm("导入会覆盖这台设备当前的练习进度。确定继续吗？")) return;
      save(imported);
      const next = makePlan("adaptive", imported, SESSION_LENGTH, curriculumScope);
  ...
  const activeKcs = kcsForScope(curriculumScope);
  ...
  const visibleCourses = coursesForScope(curriculumScope) as Course[];
  ...
  const kcStatus = (kc: KnowledgeComponent) => {
    ...
    const nonGatingParent = kc.id.startsWith("facet.class.irregular.") ? "class.irregular" : kc.id.startsWith("facet.form.") ? `suffix.${kc.id.split(".")[2]}` : kc.id === "facet.onbin.sokuon.iku" ? "onbin.sokuon" : "exception.ru-godan";
  ...
  return <main className="site-shell">
    ...
    <section className="practice-layout"><aside className="lesson-rail"><p className="eyebrow">YOKUBI 活用路线</p><h1>拆成原子，<br />逐项练会。</h1><p className="intro">系统分别跟踪词类、词干、音便、接续与例外，再把薄弱原子组合进完整活用题。</p>
      <button type="button" className={`adaptive-entry ${mode === "adaptive" ? "active" : ""}`} onClick={() => start("adaptive")}><span className="adaptive-icon">自</span><span><strong>自适应训练</strong><small>{curriculumScope === "core" ? "核心活用" : "完整路线"} · {focusDisplayLabel}</small></span><b>{focusPercent}%</b></button>
      <button ref={progressTriggerRef} type="button" className="progress-trigger" onClick={() => setProgressOpen(true)} aria-haspopup="dialog"><span><b>知识进度</b><small>{curriculumScope === "core" ? "核心活用课程与原子" : "完整课程与原子规则"}</small></span><strong>{masteredKcCount}<i>/</i>{introducedKcs.length}</strong></button>
      ...
      {result && <div className={`feedback ${result}`} role="status"><div className="feedback-copy"><strong>{feedbackTitle}</strong><p>{diagnosticMessage ?? detail.rule}</p></div><div className="knowledge-tags" aria-label="本题涉及的原子规则">{derivation.requiredKcIds.map((id: string) => KC_BY_ID.get(id)).filter((kc: KnowledgeComponent | undefined): kc is KnowledgeComponent => Boolean(kc)).map((kc: KnowledgeComponent) => <span className={kc.id === evidenceKcId ? "target" : ""} key={kc.id}>{KC_FAMILY_LABELS[kc.family]} · {kc.label}</span>)}</div><div className="rule-line"><span><FuriganaText surface={verb.surface} reading={verb.reading} /></span><b>→</b>{course.id === "classify" ? <span className="answer-emphasis">{classLabel(verb.class)}</span> : detailSteps ? detailSteps.map((step: string, i: number) => <Fragment key={`${step}-${i}`}><span className={i === detailSteps.length - 1 ? "answer-emphasis" : ""}><FuriganaText surface={step} reading={readingSteps?.[i] ?? step} /></span>{i < detailSteps.length - 1 && <b>→</b>}</Fragment>) : detail.parts.map((part: string, i: number) => <span className={i === detail.parts.length - 1 ? "answer-emphasis" : ""} key={`${part}-${i}`}><FuriganaText surface={part} reading={readingParts[i] ?? part} />{i < detail.parts.length - 1 && <b className="joiner">＋</b>}</span>)}</div>{result === "incorrect" && course.id !== "classify" && <p className="your-answer">你的答案：{answer || "—"}</p>}<div className="feedback-meta"><span>本题原子 {currentPercent}%</span><a href={course.url} target="_blank" rel="noreferrer">查看 Yokubi 中文版{course.lesson === "复习" ? "相关课程" : `第 ${Number(course.lesson)} 课`} ↗</a></div><button type="button" className="next-button" onClick={nextQuestion}>{questionNumber === SESSION_LENGTH ? "查看本轮结果" : focusComplete ? "继续" : "下一题"}<span><kbd>Enter</kbd> →</span></button></div>}{!result && <p className="keyboard-hint">{course.id === "classify" ? <><kbd>1</kbd> <kbd>2</kbd> <kbd>3</kbd> 选择答案</> : <><kbd>Enter</kbd> 检查答案</>}</p>}</article></> :
      <article className="completion-card"><p className="completion-jp">おつかれさま</p><span className="completion-label">本轮完成</span><div className="score"><strong>{sessionCorrect}</strong><span>/ {answeredInRound}</span></div><p>{unlocked ? `新原子已引入：${unlocked}` : mode === "adaptive" && activeRouteComplete ? curriculumScope === "core" ? "核心活用已经全部达标，可以继续巩固或解锁接续表达。" : "完整路线已经全部达标，可以继续巩固。" : mode === "adaptive" ? "下一轮会继续聚焦当前置信度最低的原子规则。" : "专项模式只练当前课程，不会推进自适应路线的解锁。"}</p><div className="completion-focus"><span>{mode === "adaptive" ? "当前最弱原子" : "本专项最弱原子"}</span><strong>{weakestKc?.label ?? "全部已达标"}</strong>{weakestMissingCoverage.length > 0 && <small>待覆盖：{weakestMissingCoverage.join("、")}</small>}</div><button type="button" className="restart-button" onClick={() => start(mode)}>{mode === "adaptive" ? "继续下一轮" : "继续本专项"}<span><kbd>Enter</kbd> →</span></button>{mode === "adaptive" && curriculumScope === "core" && coreComplete && <button type="button" className="unlock-curriculum" onClick={() => changeCurriculumScope("full")}>继续学习接续表达 <span>解锁 {COURSES.length - CORE_COURSE_COUNT} 门课程 →</span></button>}{mode !== "adaptive" && <button type="button" className="back-adaptive" onClick={() => start("adaptive")}>返回自适应训练</button>}</article>}

...
function PendingRetests({ assessment }: { assessment: AssessmentState }) {
  ...
  return <section className="pending-retests" aria-label="待独立复测">
    ...
    {Object.values(assessment.suspendedPending ?? {}).length > 0 && <details><summary>已暂停复测 {Object.keys(assessment.suspendedPending).length} 项</summary><p>当前题库没有适用的同规则题目。这些任务保留历史记录，不计为通过，也不阻碍继续学习；以后题库恢复适用题目时会继续复测。</p>{Object.values(assessment.suspendedPending).map(entry => <p key={entry.key}>{entry.target.surface} · {entry.target.form ? FORM_LABELS[entry.target.form as Form] : "词类判断"}</p>)}</details>}
    {pending.map(entry => {
...
type AssignedQuestion = {item: KnowledgeComponent; candidate: Exercise};
...
export default function Home() {
  ...
  const pendingInMode = pendingRetests.filter(entry => challenging ? activeChallengeCourses.includes(entry.courseId as ModeId) : mode === "adaptive" || entry.courseId === mode);
  ...
  const roundQuestionLimit = challenging ? roundQuestions.length : verification?.exercise || Object.values(planningProfile.assessment.pending).some(entry => mode === "adaptive" || entry.courseId === mode)
  ...
  const focusComplete = !challenging && Boolean(result && !currentPending && focusKc && isComponentMastered(focusKc, profile.byKc));
  ...
  return <main className="site-shell">
    ...
    <div hidden={showStatistics || showChallengeSelection} inert={showStatistics || showChallengeSelection}><section className={`practice-layout ${challenging ? "challenge-layout" : ""}`}>{!challenging && <aside className="lesson-rail"><p className="eyebrow">YOKUBI 活用路线</p><h1>拆开规律，<br />逐项练会。</h1><p className="intro">先掌握动词与形容词的基本词形，再按表达目标学习，最后综合运用。</p>
      ...
      challenging ? <article className="completion-card"><p className="completion-jp">おつかれさま</p><span className="completion-label">本轮挑战完成</span>{answeredInRound > 0 && <div className="score"><strong>{sessionCorrect}</strong><span>/ {answeredInRound}</span></div>}<p>{challengeSummary}</p><p>作答已记录。可以继续挑战，或返回自适应训练补练。</p><button type="button" className="restart-button" onClick={() => startChallenge(activeChallengeCourses)}>{activeChallengeCourses.length > 1 ? "继续挑战所选课程" : "继续挑战本课"}<span><kbd>Enter</kbd> →</span></button><button type="button" className="back-adaptive" onClick={openChallenge}>更换挑战课程</button><button type="button" className="back-adaptive" onClick={() => start("adaptive")}>返回自适应训练</button></article> :

...
function DiagnosticPractice({ item, steps, onEvidence, onDone }: {
  ...
  return <section className="diagnostic-practice" aria-label="拆步练习">
    ...
    {feedback && <div role="status"><p>{feedback}</p><p>本步正确形式：<FuriganaText surface={step.answers[0]} reading={step.readings[0]} /></p><button ref={nextButton} type="button" className="text-button" data-diagnostic-next aria-keyshortcuts="Enter" onClick={next}>{index === steps.length - 1 ? "完成拆步" : "练习下一步"} <kbd aria-hidden="true">Enter</kbd></button></div>}
...
export default function Home() {
  ...
  function submit(event: FormEvent) {
    ...
    const diagnosed: { kcId: string; message: string; confirmedKcIds?: string[] } | null = item.domain === "adjective"
    ...
    if (!diagnosed && hasLexicalTypo(item, answer, derivation.acceptedVariants, readingDerivation.acceptedVariants, normalize)) {
      setTypoNotice(true);
  ...
  const courseSummary = (item: Course) => {
    ...
    return summarizeCourseProgress(item, required, introducedSet, profile.byKc);
  };

...
type ModeId = "classify" | "negative" | "past" | "te" | "giving" | "request" | "imperative" | "masu" | "aspect" | "passive" | "potential" | "volitional" | "desire" | "ba" | "tara" | "nasai" | "prohibitive" | "temo" | "concurrent" | "obligation" | "listing" | "permission" | "youtosuru" | "temiru" | "causative" | "causativePassive" | "nakuteNaide" | "zuZuni" | "teshimauChau" | "teokuToku" | "direction" | "tatte" | "sugiru" | "tagaru" | "basicCompound" | "voiceCompound" | "adjectiveClassify" | "adjectiveIBase" | "adjectiveITe" | "adjectiveNaBase" | "adjectiveConditional" | "adjectiveAdverb";
...
export default function Home() {
  ...
  function grade(correct: boolean, revealed = false, failedKcId: string | null = null, message: string | null = null) {
    ...
    const byKc = updateKnowledgeStats(old.byKc, { kcIds: derivation.requiredKcIds, focusId: targetKc.id, failedKcId, correct, revealed, hintUsed: hintShown, responseMs: clockNow() - startedAt.current, answerLength: form ? conjugateFor(readingItem, form).length : item.reading.length });
  ...
  function submit(event: FormEvent) {
    ...
    const accepted = [...derivation.acceptedVariants, ...deriveFor(readingItem, form).acceptedVariants].map(normalize);
  ...
  return <main className="site-shell">
    ...
    <section className="practice-layout"><aside className="lesson-rail"><p className="eyebrow">YOKUBI 活用路线</p><h1>拆成原子，<br />逐项练会。</h1><p className="intro">{practiceDomain === "verb" ? "系统分别跟踪词类、词干、音便、接续与例外，再把薄弱原子组合进完整活用题。" : "系统分别跟踪形容词分类、词尾、接续、复合与例外，再把薄弱原子组合进完整活用题。"}</p>
      ...
      <button ref={progressTriggerRef} type="button" className="progress-trigger" onClick={() => setProgressOpen(true)} aria-haspopup="dialog"><span><b>知识进度</b><small>{practiceDomain === "adjective" ? "形容词课程与原子" : curriculumScope === "core" ? "核心活用课程与原子" : "完整课程与原子规则"}</small></span><strong>{masteredKcCount}<i>/</i>{introducedKcs.length}</strong></button>
      ...
      {result && <div className={`feedback ${result}`} role="status"><div className="feedback-copy"><strong>{feedbackTitle}</strong><p>{diagnosticMessage ?? detail.rule}</p></div><div className="knowledge-tags" aria-label="本题涉及的原子规则">{derivation.requiredKcIds.map((id: string) => KC_BY_ID.get(id)).filter((kc: KnowledgeComponent | undefined): kc is KnowledgeComponent => Boolean(kc)).map((kc: KnowledgeComponent) => <span className={kc.id === evidenceKcId ? "target" : ""} key={kc.id}>{KC_FAMILY_LABELS[kc.family]} · {kc.label}</span>)}</div><div className="rule-line"><span><FuriganaText surface={item.surface} reading={item.reading} /></span><b>→</b>{!form ? <span className="answer-emphasis">{classLabelFor(item.class)}</span> : detailSteps ? detailSteps.map((step: string, i: number) => <Fragment key={`${step}-${i}`}><span className={i === detailSteps.length - 1 ? "answer-emphasis" : ""}><FuriganaText surface={step} reading={readingSteps?.[i] ?? step} /></span>{i < detailSteps.length - 1 && <b>→</b>}</Fragment>) : detail.parts.map((part: string, i: number) => <span className={i === detail.parts.length - 1 ? "answer-emphasis" : ""} key={`${part}-${i}`}><FuriganaText surface={part} reading={readingParts[i] ?? part} />{i < detail.parts.length - 1 && <b className="joiner">＋</b>}</span>)}</div>{result === "incorrect" && form && <p className="your-answer">你的答案：{answer || "—"}</p>}<div className="feedback-meta"><span>本题原子 {currentPercent}%</span><a href={course.url} target="_blank" rel="noreferrer">查看 Yokubi 中文版{course.lesson === "复习" ? "相关课程" : `第 ${Number(course.lesson)} 课`} ↗</a></div><button type="button" className="next-button" onClick={nextQuestion}>{questionNumber === SESSION_LENGTH ? "查看本轮结果" : focusComplete ? "继续" : "下一题"}<span><kbd>Enter</kbd> →</span></button></div>}{!result && <p className="keyboard-hint">{!form ? <>{classChoices.map((_, index) => <Fragment key={index}><kbd>{index + 1}</kbd>{" "}</Fragment>)}选择答案</> : <><kbd>Enter</kbd> 检查答案</>}</p>}</article></> :
      <article className="completion-card"><p className="completion-jp">おつかれさま</p><span className="completion-label">本轮完成</span><div className="score"><strong>{sessionCorrect}</strong><span>/ {answeredInRound}</span></div><p>{unlocked ? `新原子已引入：${unlocked}` : mode === "adaptive" && activeRouteComplete ? practiceDomain === "adjective" ? "形容词核心活用已经全部达标，可以继续巩固。" : curriculumScope === "core" ? "核心活用已经全部达标，可以继续巩固或解锁接续表达。" : "完整路线已经全部达标，可以继续巩固。" : mode === "adaptive" ? "下一轮会继续聚焦当前置信度最低的原子规则。" : "专项模式只练当前课程，不会推进自适应路线的解锁。"}</p><div className="completion-focus"><span>{mode === "adaptive" ? "当前最弱原子" : "本专项最弱原子"}</span><strong>{weakestKc?.label ?? "全部已达标"}</strong>{weakestMissingCoverage.length > 0 && <small>待覆盖：{weakestMissingCoverage.join("、")}</small>}</div><button type="button" className="restart-button" onClick={() => start(mode)}>{mode === "adaptive" ? "继续下一轮" : "继续本专项"}<span><kbd>Enter</kbd> →</span></button>{mode === "adaptive" && practiceDomain === "verb" && curriculumScope === "core" && coreComplete && <button type="button" className="unlock-curriculum" onClick={() => changeCurriculumScope("full")}>继续学习接续表达 <span>解锁 {VERB_COURSES.length - CORE_COURSE_COUNT} 门课程 →</span></button>}{mode !== "adaptive" && <button type="button" className="back-adaptive" onClick={() => start("adaptive")}>返回自适应训练</button>}</article>}

...
export function createChallengePlanner(model) {
  ...
  function questions(selection,profile,{length=12,seed=0}={}) {
    ...
    const groups=new Map(),courses=selected.map(courseId=>{
      ...
      return {id:courseId,forms:[...forms.values()].map(classes=>[...classes]
        .sort(([a],[b])=>a<b?-1:a>b?1:0).map(([,target])=>target))};
    });
    ...
    for(let index=0;index<length;index++) {
      ...
      const previous=courses.length===1?rotation:Math.floor((rotation*length+ordered.indexOf(course))/courses.length);
